...
function assertEq(actual, expected, msg) {
  ...
  if (a === e) { passed++; } else { failed++; console.error("FAIL:", msg, "expected:", e, "got:", a); }
}
...
for (const pl of ALL_L_PLACEMENTS) {
  ...
  assert(pl.every(p => p[0] >= 0 && p[0] < 4 && p[1] >= 0 && p[1] < 4), "all in bounds");
}
...
const symResults = SYMMETRIES.map(s => {
  return [[0,0],[0,1],[1,0],[3,3]].map(p => s(p[0], p[1]).join(",")).join(";");
});
assert(new Set(symResults).size === 8, "all 8 symmetries produce distinct results");

for (let si = 0; si < 8; si++) {
  const corners = [[0,0],[0,3],[3,0],[3,3]];
  for (const [c, r] of corners) {
    const [tc, tr] = SYMMETRIES[si](c, r);
    assert(corners.some(cr => cr[0] === tc && cr[1] === tr),
      `symmetry ${si} maps corner (${c},${r}) to corner`);
  }
}

// --- Canonicalization ---
const active1 = sortCells([[0,0],[0,1],[0,2],[1,0]]);
const opponent1 = sortCells([[3,3],[3,2],[3,1],[2,3]]);
const neutrals1 = sortCells([[1,3],[2,0]]);
...
for (let si = 0; si < 8; si++) {
  ...
  assertEq(canonicalize(ta, to, tn), canon1, `symmetry ${si} gives same canonical key`);
}

// --- Losing positions count ---
assert(LOSING_POSITIONS.size === 29, `expected 29 losing positions, got ${LOSING_POSITIONS.size}`);
...
const rotResult = isLosingPosition(
  sortCells([[0,0],[0,1],[0,2],[1,0]].map(p => sym(p[0], p[1]))),
  sortCells([[1,1],[1,2],[1,3],[2,1]].map(p => sym(p[0], p[1]))),
  sortCells([[2,0],[3,1]].map(p => sym(p[0], p[1])))
);
...
const rot90opponent = sortCells(opponent1.map(p => SYMMETRIES[1](p[0], p[1])));
const rot90neutrals = sortCells(neutrals1.map(p => SYMMETRIES[1](p[0], p[1])));
assert(
  isIsomorphic(active1, opponent1, neutrals1, rot90active, rot90opponent, rot90neutrals),
  "90-degree rotation is isomorphic"
);

const rot180active = sortCells(active1.map(p => SYMMETRIES[2](p[0], p[1])));
const rot180opponent = sortCells(opponent1.map(p => SYMMETRIES[2](p[0], p[1])));
const rot180neutrals = sortCells(neutrals1.map(p => SYMMETRIES[2](p[0], p[1])));
assert(
  isIsomorphic(active1, opponent1, neutrals1, rot180active, rot180opponent, rot180neutrals),
...
assert(
  isIsomorphic(active1, opponent1, neutrals1, reflActive, reflOpponent, reflNeutrals),
  "reflection is isomorphic"
);

const differentActive = sortCells([[0,0],[0,1],[1,1],[2,1]]);
assert(
  !isIsomorphic(active1, opponent1, neutrals1, differentActive, opponent1, neutrals1),
  "different positions are not isomorphic"
);

// --- generateMoves ---
const startActive = sortCells([[1,0],[1,1],[1,2],[2,0]]);
const startOpponent = sortCells([[2,1],[2,2],[2,3],[3,3]]);
const startNeutrals = sortCells([[3,0],[0,3]]);
...
for (const m of moves) {
  ...
  assert(unique.size === allCells.length, "no overlapping pieces in move result");
}
...
const noNeutralMoves = moves.filter(m =>
  m.neutrals[0][0] === startNeutrals[0][0] && m.neutrals[0][1] === startNeutrals[0][1] &&
  m.neutrals[1][0] === startNeutrals[1][0] && m.neutrals[1][1] === startNeutrals[1][1]
);
...
const lostActive = [[0,0],[0,1],[0,2],[1,0]];
const lostOpponent = [[1,1],[1,2],[1,3],[2,1]];
const lostNeutrals = [[2,0],[3,1]];
...
const d2Active = [[0,0],[0,1],[0,2],[1,0]];
const d2Opponent = [[1,1],[1,2],[2,1],[3,1]];
const d2Neutrals = sortCells([[0,3],[2,0]]);
const d2Result = isLosingPosition(d2Active, d2Opponent, d2Neutrals);
assert(d2Result !== null && d2Result.movesLeft === 1, "depth-2 position confirmed");
...
assert(allMovesLeadToOpponentWin, "every move from depth-2 lets opponent force depth-0 loss");
